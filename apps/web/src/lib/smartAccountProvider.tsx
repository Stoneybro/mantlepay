...
export function SmartAccountProvider({ children }: { children: React.ReactNode }) {
  ...
  const initialize = useCallback(
    async (force = false): Promise<SmartAccountClient | null> => {
      // Return existing promise if inflight and not forcing a new init
      ...
      const promise = (async (): Promise<SmartAccountClient | null> => {
        ...
        if (!ready || !authenticated || !embeddedWallet) {
          console.debug("[SmartAccount] init skipped: auth not ready");
          return null;
        }

        if (customError) {
          console.error("[SmartAccount] custom account error", customError);
