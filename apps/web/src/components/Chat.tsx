...
function Chat() {
  ...
  // Transaction hooks
  const singleEthTransfer = useSingleTransfer();
  const singlePyusdTransfer = useSingleTokenTransfer();
  const batchEthTransfer = useBatchTransfer();
  const batchPyusdTransfer = useBatchTokenTransfer();
  const recurringEthPayment = useRecurringPayment();
  const recurringPyusdPayment = useRecurringTokenPayment();
          ...
          case 'output-available':
            return (
              <div key={callId} className="my-2 text-sm flex flex-col">
                ...
                <div>Hash: <a href={`https://sepolia.arbiscan.io/tx/${part.output}`} className="underline" target="_blank" rel="noopener noreferrer">{part.output as string}</a></div>
              </div>
            );
            
          case 'output-error':
            return (
              <div key={callId} className="my-2 flex items-center gap-1 text-sm text-destructive">
    ...
    if (part.type === "tool-executeBatchEthTransfer") {
      if ('state' in part) {
        switch (part.state) {
          ...
          case 'output-error':
            return (
              <div key={callId} className="my-2 text-sm flex items-center gap-1 text-destructive">
                <FaTimes /> Error: {part.errorText}
              </div>
            );
        }
            ...
            return (
              <div key={callId} className="my-4 p-4 border bg-background rounded-lg">
                ...
                <div className="text-sm space-y-1 mb-4">
                  ...
                  <p className="text-destructive mt-2">⚠️ This action cannot be undone. The recurring payment will stop immediately.</p>
                </div>
