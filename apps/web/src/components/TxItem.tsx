...
import { formatEth, formatNumber, formatTimestamp, truncateAddress } from "@/utils/format";
    ...
    switch (tx.type) {
      ...
      case TransactionType.CONTRACT_CALL:
        return (
          <div className="space-y-1.5">
            ...
            {tx?.label && (
              <div className="text-xs text-muted-foreground">
                <span className="text-muted-foreground/70">Type:</span> {tx.label}
              </div>
            )}
            {tx.value > 0n && (
      ...
      case TransactionType.EXECUTE_BATCH:
        return (
          <div className="space-y-1.5">
            <div className="text-xs text-muted-foreground">
              <span className="text-muted-foreground/70">Recipient count:</span> {tx.batchSize.toString()}
            </div>
            <div className="text-sm font-medium">
      ...
      case TransactionType.INTENT_CREATED:
        return (
          <div className="space-y-1.5">
            ...
            <div className="text-xs text-muted-foreground">
              <span className="text-muted-foreground/70">Recipients:</span> {tx.recipientCount} 
            </div>

            <div className="mt-2 space-y-1 rounded-md border px-3 py-2 bg-muted/30">
              <div className="text-xs">
                <span className="text-muted-foreground/70">Total Payments:</span> {tx.totalTransactionCount.toString()}
              </div>
              <div className="text-xs">
                <span className="text-muted-foreground/70">Duration:</span> {formatDuration(tx.duration)}
              </div>
              <div className="text-xs">
                <span className="text-muted-foreground/70">Interval:</span> {formatDuration(tx.duration)}
              </div>
            </div>
          </div>
        );
          ...
          <div className="space-y-1.5">
            <div className="text-sm font-semibold">
             Payment Name: {tx.intentName || 'Unnamed Schedule'}
                         ...
                         <div className="text-xs font-medium text-muted-foreground">Recipients: {tx.recipientCount}</div>
            <div className="mt-2 space-y-2 rounded-md border px-3 py-2 bg-muted/30">

                {/* {tx.recipients.map((recipient, index) => (<div className=""> {truncateAddress(recipient.address)} </div>))} */}
               </div>
